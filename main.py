...
async def conn_han(client):
    await client.subscribe('alex/temperatura', 1)
    await client.subscribe('alex/humedad', 1)
    ...
    while True:
        try:
            ...
            try:
                ...
                await client.publish('alex/temperatura', '{}'.format(temperatura), qos = 1)
            except OSError as e:
                print("sin sensor temperatura")
            try:
                humedad=d.humidity()
                await client.publish('alex/humedad', '{}'.format(humedad), qos = 1)
            except OSError as e:
